...
class TeamResource:
    ...
    def add_team(team_name, team_captain, course_id, number_needed=0, team_message=""):
        if not (team_name and course_id):
            return False, "Please fill in all blanks!"
        ...
        sql1 = """
                ...
                WHERE Team_Name = %s and Course_id = %s
                """
        cur.execute(sql1, args=(team_name, course_id))
        records = cur.fetchall()
        if len(records) >= 1:
            return False, "DUPLICATED TEAM"

        sql2 = """
                 insert into teammate_db.Team (Team_Name, Team_Captain, Course_id, Number_needed, Team_message)
                 values (%s, %s, %s, %s, %s);
                """
        cur.execute(sql2, args=(team_name, team_captain, course_id, number_needed, team_message))
        result = cur.rowcount
        return True if result == 1 else False

    @staticmethod
    def edit_team(team_name, team_captain, course_id, number_needed=0, team_message=""):
        if not (team_name and team_captain and course_id):
            return False
        conn = TeamResource._get_connection()
        team_name, team_captain, course_id = team_name.strip(), team_captain.strip(), course_id.strip()
        cur = conn.cursor()
        sql1 = """
            SELECT * FROM teammate_db.Team where team_name = %s and course_id = %s
            """
        res = cur.execute(sql1, args=(team_name, course_id))
        records = cur.fetchall()
        if len(records) < 1:
            return False
        sql2 = """
            UPDATE teammate_db.Team 
            set team_captain = %s, number_needed = %s, team_message = %s
            where team_name = %s and course_id = %s
            """;
        cur.execute(sql2, args=(team_captain, number_needed, team_message, team_name, course_id))
        return True
    ...
    def delete_team(team_id, course_id):
        if not (course_id and team_id):
            return False
        sql1 = """
        SELECT * FROM teammate_db.Team where Team_id = %s and Course_id = %s
        """
        conn = TeamResource._get_connection()
        cur = conn.cursor()
        res = cur.execute(sql1, args=(team_id, course_id))
        records = cur.fetchall()
        if len(records) < 1:
            return False
        sql2 = "DELETE FROM teammate_db.Team where Team_id = %s and Course_id = %s";
        cur.execute(sql2, args=(team_id, course_id))
        return True
